...
# ============================================================================
# C.21 experiment: Relax 1D trend filter — remove close > EMA50_1d gate
#
# Live dry-run finding (June 2026): 0 trades in 60 days across 10 pairs.
# DC55v24 bull_1d = (ema50_1d > ema200_1d) AND (close_1d > ema50_1d).
#
# Root cause: In a ranging/corrective market after a bull run, altcoins pull
# back below EMA50_1d while the golden cross remains intact. DC55v24's strict
# "close > EMA50_1d" condition blocks ALL long entries in exactly these periods:
#   - close < ema50_1d → no long (condition 2 fails)
#   - ema50_1d > ema200_1d → no short (death cross not active)
#   Result: complete signal blackout in sideways/correction periods.
#
# DC55v31 fix: remove close_1d > ema50_1d requirement.
#   long  = golden cross only  (ema50_1d > ema200_1d)
#   short = death cross only   (ema50_1d < ema200_1d)
# All other quality gates unchanged: ADX>15, ATR expansion, REL_VOL>=2.0,
# 55-bar Donchian, RSI<45 for shorts, SUSHI/ATOM/YFI/CRV blacklist.
# ============================================================================


class DC55v31(DC55v24):
    """
    DC55v24 with relaxed 1D trend filter (C.21).

    Removes close_1d > EMA50_1d from the regime gate.
    DC55v24 requires BOTH golden cross AND price above its 50-day EMA,
    which blocks entries during bull-market pullbacks where price temporarily
    dips below EMA50_1d while the overall uptrend (golden cross) remains intact.

    DC55v31 regime gates:
      long  = ema50_1d > ema200_1d  (golden cross only)
      short = ema50_1d < ema200_1d  (death cross only)

    Unchanged: ADX(4H)>15, ATR expansion, REL_VOL>=2.0, 55-bar Donchian,
    RSI(4H)<45 for shorts (asymmetric), blacklist (SUSHI/ATOM/YFI/CRV).
    """
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        live       = dataframe["volume"] > 0
        vol_ok     = dataframe["rel_vol"] >= self.REL_VOL
        bull_1d    = dataframe["ema50_1d"] > dataframe["ema200_1d"]   # golden cross only
        bear_1d    = dataframe["ema50_1d"] < dataframe["ema200_1d"]   # death cross only
        atr_expand = dataframe[f"atr14_{itf}"] > dataframe[f"atr14_sma20_{itf}"]
        adx_ok     = dataframe[f"adx_{itf}"] > self.ADX_THRESHOLD     # 15
        rsi_short  = dataframe[f"rsi14_{itf}"] < 45                   # shorts only
